# CCS Server Preparation Guide

This document provides the **basic preparation requirements** for deploying the **CCS Web Server** on Microsoft Windows Server.
...
**Visual Studio Code (VS Code)** is a free, lightweight source-code editor developed by Microsoft. It can be used to inspect and configure the CCS Web Server source code.
...
**Node.js** provides the JavaScript runtime environment required to run the CCS Web Server.
...
Install or prepare the software downloaded in the previous step:
...
After installation, verify that Node.js and npm are available from PowerShell:
...
## 1.8 Configure DNS for CCS Web Server 

In your DNS Hosted Zone, create or update the required DNS record so that the CCS Web Server hostname resolves to the server's **public static IP address**.
...
## 1.9 Download the CCS Web Server Source Code

Download the official CCS Web Server source code from the XSTRING TECH software downloads repository.
...
You do not need to download or install xstAuth Light Server or the licence certificate on your CCS Web Server

> **Security recommendation:**
> Obtain production software only from the official XSTRING TECH software release repository. Verify the release version and any published checksums before installation where applicable.
...
The **HTTPS-01.js** and **env-example.txt** files will be used at a later stage.
...
* Confirm that `HTTP-01.js` is running in VS Code.
* Test the URL again.

Do not proceed with the TLS certificate request until the HTTP test is successful.

If the test page loads successfully, keep the HTTP server running and proceed to use **win-acme (WACS)** to obtain the TLS certificate required by the **CCS Web Server**.
...
1.  **CCS Web Server** --- one certificate for the CCS host.

The examples below use:

-   CCS Web Server: `www.yourdomain.com`
...
``` text
[www.yourdomain.com] Authorizing...
[www.yourdomain.com] Authorizing using http-01 validation (FileSystem)
Answer should now be browsable at:
http://www.yourdomain.com/.well-known/acme-challenge/<challenge-file>

Preliminary validation looks good...
[www.yourdomain.com] Authorization result: valid
```
...
C:/xstAuth/certs/www.yourdomain.com-key.pem
C:/xstAuth/certs/www.yourdomain.com-crt.pem
C:/xstAuth/certs/www.yourdomain.com-chain-only.pem
C:/xstAuth/certs/www.yourdomain.com-chain.pem
```
...
Before testing HTTPS, ensure that HTTPS traffic is allowed through the **cloud/network firewall** and **Windows Firewall** for **CCS Web Server**.
...
Check that **key path**, and **certificate path(s)** have been entered.
...
After completing **Step 2**, you should have successfully configured and verified the **HTTPS connection for CCS Web Server**.
